...
import { Loader2, Calendar, Clock, MapPin, User as UserIcon, CheckCircle, XCircle } from 'lucide-react';
...
export default function BookingDetailsPage() {
    ...
    if (isLoading) return <div className="min-h-screen flex flex-col"><Header /><div className="flex-1 flex justify-center items-center"><Loader2 className="h-8 w-8 animate-spin" /></div><Footer /></div>;
    if (!booking) return <div className="min-h-screen flex flex-col"><Header /><div className="flex-1 flex justify-center items-center">Booking not found</div><Footer /></div>;

    const isCustomer = user?.id === booking.customerId; // Or check role 'CUSTOMER'
    const isProvider = user?.id === booking.providerId; // Or check role 'PROVIDER'

    // Determine available actions based on user booking status logic
    const renderActions = () => {
        if (isCustomer) {
            if (['REQUESTED', 'ACCEPTED'].includes(booking.status)) {
                return (
                    <Button variant="destructive" onClick={() => updateStatus({ id, status: 'CANCELLED' })} disabled={isUpdating}>
                        Cancel Booking
                    </Button>
                );
            }
        ...
        <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950">
            ...
            <main className="flex-1 container mx-auto px-4 py-8">
                <div className="mb-6 flex items-center justify-between">
                    <Link href="/bookings" className="text-sm text-slate-500 hover:text-slate-900">&larr; Back to Bookings</Link>
                    <span className="px-3 py-1 rounded-full text-sm font-semibold bg-white border border-slate-200 shadow-sm">{booking.status}</span>
                </div>
                            ...
                            <div className="grid sm:grid-cols-2 gap-6 mb-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center text-slate-600 dark:text-slate-300">
                                        ...
                                        <span>{isCustomer ? `Provider: ${booking.provider.firstName}` : `Customer: ${booking.customer.firstName}`}</span>
                                    </div>
                                    <div className="flex items-center text-slate-600 dark:text-slate-300">
